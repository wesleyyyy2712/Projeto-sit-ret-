  ...
  return (
    <div className="ret-app">
      ...
      <main id="inicio">
        ...
        <section className="catalog" id="catalogo"><div className="catalog-title"><div><span className="section-label">{t.official}</span><h2>{t.catalogTitle}</h2></div><button onClick={() => setActiveCategory("Todos")}>{t.all} <ArrowRight size={14} /></button></div><div className="categories">{["Informação", ...categories].map((category) => <button key={category} className={activeCategory === category ? "active" : ""} onClick={() => setActiveCategory(category)}>{category}</button>)}</div><div className="info-panel">
              <div className="info-art"><img src="/retzada-cheats.png" alt="RETZADA Cheats" /><span className="info-art-glow" /></div>
              <div className="info-copy">
                <span className="section-label">RET CHEATS / INFORMAÇÃO</span>
                <h3>Seu acesso começa aqui.</h3>
                <p>Conheça a RET CHEATS, veja as novidades do catálogo e fale diretamente com a nossa equipe para receber valores, disponibilidade e suporte.</p>
                <div className="info-points"><span><ShieldCheck size={17} /> Compra segura e atendimento direto</span><span><Zap size={17} /> Produtos atualizados e suporte rápido</span><span><MessageCircle size={17} /> Escolha uma categoria para ver os produtos</span></div>
                <button className="hero-button" onClick={() => setActiveCategory("Todos")}>Ver produtos <ArrowRight size={16} /></button>
              </div>
            </div>
